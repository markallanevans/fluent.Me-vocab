...
const sentenceStore = (
  ...
  switch (action.type) {
    ...
    case 'LOAD_SENTENCE': {
      const { loadedCategory, currentWordIndex } = state
      return {
        ...state,
    ...
    case 'ADD_NEW_WORD_TO_STORE': {
      ...
      categories.forEach(category => {
        allCategories[category].listWords = allCategories[
          category
        ].listWords.concat(action.word)
        if ('sentences' in allCategories[category])
          allCategories[category].sentences[action.word] = action.sentence
      })
      return {
        ...state,
        allCategories
